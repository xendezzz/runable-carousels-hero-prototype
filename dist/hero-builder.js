(() => {
  ...
  source.setAttribute('aria-label','Describe your carousel idea');
  ...
  form.hidden = true;
  form.setAttribute('aria-label', 'Describe your carousel');
  form.innerHTML = `<button type="button" class="builder-close" aria-label="Close carousel idea input">×</button><textarea id="website-idea" aria-label="Your carousel idea" placeholder="Describe the carousel you want to create…" maxlength="2000" rows="2" required></textarea><div class="builder-actions"><span class="builder-hint" aria-hidden="true">press enter <kbd>↵</kbd></span><button type="submit" class="builder-submit" aria-label="Create carousel" disabled><img src="assets/hero/submit.svg" alt=""></button></div><p class="builder-status" role="status"></p>`;
  hero.append(form);
